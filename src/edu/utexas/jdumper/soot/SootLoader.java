...
public class SootLoader
{
    ...
    public static List<SootClass> loadSootClassesFromJimple(String jimpleDir)
    {
        JimpleClassProvider provider = setJimpleClassProvider(jimpleDir);
        return loadClasses(provider);
    }

    private static JimpleClassProvider setJimpleClassProvider(String jimpleDir)
    {
        JimpleClassProvider provider = new JimpleClassProvider();
        try
        {
            File[] files = new File(jimpleDir).listFiles();
            for (File file: files)
            {
                if (file.getName().endsWith(".jimple"))
                    provider.addClass(file);
                else
                    System.err.println("Warning: found non-jimple file \'" + file.getName() + "\' under directory \'" + jimpleDir + "\'");
            }
        } catch (IOException e)
        {
    ...
    private static List<SootClass> loadClasses(JimpleClassProvider provider)
    {
        // Force resolving all classes
        Options.v().set_full_resolver(true);
        // Do NOT allow any phantom classes
        ...
        System.out.println("[JimpleDumper] Loading classes from JIMPLE files...");
        Scene scene = Scene.v();
        for (String className: provider.getClassNames())
        {
            scene.loadClass(className, SootClass.SIGNATURES);
